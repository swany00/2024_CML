...
for i in range(len(geo_data['features'])):
    ...
    # 전체 데이터의 평균값을 계산
    avg_value = tdf[tdf['Area Code (ISO2)'] == geo_data['features'][14]['properties']['ISO']]['Value'].mean()
    
    # 툴팁 내용 설정 (예: 평균값만 표시)
    tooltip_content = f'{area_code}: 15년 {avg_value:.2f}'
